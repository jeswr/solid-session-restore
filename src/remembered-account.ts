...
export class RememberedAccount {
   ...
   */
  constructor(storageKey: string = DEFAULT_REMEMBERED_ACCOUNT_KEY) {
    this.#key = storageKey;
  }
  ...
  read(): RememberedAccountRecord | null {
    ...
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw) as Partial<RememberedAccountRecord>;
  ...
  write(webId: string, issuer: string): void {
    try {
      globalThis.localStorage?.setItem(
        this.#key,
        JSON.stringify({ webId, issuer } satisfies RememberedAccountRecord),
      );
    } catch {
